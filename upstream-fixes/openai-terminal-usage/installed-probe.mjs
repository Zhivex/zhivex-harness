...
try{
 ...
 console.log(JSON.stringify({passed,failed,toolExecutions,inputTokens:diagnostics?.budget?.inputTokens,outputTokens:diagnostics?.budget?.outputTokens,usageComplete:diagnostics?.budget?.usageComplete,network:'mock-only',packages:'installed local tarballs'}));
