...
describe("release provenance", () => {
  ...
  test("accepts a workflow dispatch from the exact version tag", () => {
    expect(verify("refs/tags/v0.9.0")).toBe("refs/tags/v0.9.0");
  });
  ...
  test.each([
    ...
    expect(() => verify(ref)).toThrow(
      "provenance workflow ref must be refs/heads/main or refs/tags/v0.9.0"
    );
  });

  test("still rejects provenance bound to another source commit", () => {
    const statement = statementFor("refs/tags/v0.9.0");
