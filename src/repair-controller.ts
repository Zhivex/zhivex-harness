...
import { z } from "zod";
import type { GenerateResult, LanguageModelMiddleware, ModelGenerateInput, StreamEvent, ToolCall, ToolSet } from "@zhivex-ai/core";
...
export const createRepairController = (metadata: Record<string, unknown>, oci: boolean) => {
  ...
  const middleware: LanguageModelMiddleware = {
    ...
    async wrapGenerate(context, next) { const call = prepare(context.input, context.model.provider); return call ? synthetic(call) : next(); },
  ...
  return { state, snapshot, pending, closure, markIncomplete, verificationFailed, wrapTools, middleware };
};
