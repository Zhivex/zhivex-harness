...
The `live-certification` GitHub environment does not currently contain provider credentials. This is not a paid GitHub feature, but adding credentials transfers sensitive provider keys into GitHub and therefore requires an explicit maintainer decision.

The release workflow fails closed when those credentials are absent: future npm publication depends on the base, orchestration, routing, and model-directed OCI live gates running against the exact annotated release tag and source commit. When credentials are configured, they are exposed only to the final provider-call step; checkout, runtime setup, dependency installation, tag verification, and deterministic OCI validation do not receive them. Local credentials are never copied to GitHub automatically.
