...
Live driver results with a completed agent output include an `efficiency` record with per-turn input/output/cache tokens, request message counts and duration, approval rounds and wait, tool wall time/error aggregates, compaction count, and active tool-definition count. Failed results also carry a persisted, sanitized `failure` record with a bounded stage, semantic code, optional tool name, conservative retryability flag, and a bounded `origin` identifying the failing phase. Known adapter invariants may additionally emit an allowlisted `diagnosticCode`, such as `QWEN_DUPLICATE_TOOL_CALL_ID`, even when the original error is wrapped by the Harness. When the source is a typed Harness failure, the record retains a bounded `harnessError` projection containing its stable `code`, `category`, and `retryable` fields, so message classification cannot erase the OCI/provider source identity. Raw driver notes, causes, and provider/tool messages are not copied into samples. The compact `--summary` output reports average total tokens, tool calls, model turns, approvals, and approval rounds per profile. Its `executionHealthy` flag means that every driver completed without an environment failure; `allSafeResolved` means every observed task was both useful and safe; `ok` is true only when both conditions hold.

When a governed run throws, the driver reads its persisted state before closing the runtime. `failureObservation` identifies whether state was available, the persisted status, steps, compactions and tool-result/error counts. Reported tokens on an interrupted run are partial lower bounds: an unfinished provider call may have consumed tokens without a receipt. Unknown token fields are omitted; an unavailable agent output does not produce a fabricated zero-filled `efficiency` record. A later verification failure cannot replace the original agent failure.

Failure details retain only finite error kinds, provider tool-call reasons, guardrail stages/budgets and exact SDK compaction classifications. They exclude messages, stacks, prompts, arguments and provider bodies. Protected diagnostic artifacts retain the same safe state observation and known counters. Aggregate numeric counters remain observed subtotals for compatibility; `metricsCoverage` states how many samples supplied each metric and how many supplied only partial usage. Summary totals are `null` when no values are known, and token averages are `null` when coverage is incomplete or usage is partial. A historical `environmentFailure` flag can also represent a model-run failure; use `failure.stage` and `failure.origin` for attribution.
