...
The source version is `1.1.2`, targeting npm `latest` after fresh certification.
...
Its tag and evidence remain unchanged. All exact-artifact gates must pass anew for `v1.1.2`.
`release-status.json` continues to describe the independently verified 1.0.0
publication until 1.1.2 is published and verified; RC evidence is not substituted
