...
2. Preserve the failed, unpublished `1.0.0-rc.1` attempt and its protected-workflow evidence; never retag or reuse it.
3. Publish the forward-fixed `1.0.0-rc.2` to `next` and run migration, representative repository, live provider, OCI, installed-artifact, integrity, and provenance gates against that exact tag.
4. Complete the observation cycle, publish an independent `1.0.0-rc.3` to `next`, and repeat the full matrix so RC.2 and RC.3 provide two passing candidates.
