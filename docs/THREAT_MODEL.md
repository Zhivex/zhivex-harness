...
Personal CLI profiles live outside the repository, contain only provider/model identity, and are read with strict schema, size, permission, link, and descriptor-continuity checks. A bare interactive console automatically activates an existing user-owned `default` profile when no explicit selection, provider/model environment override, or resumed session is present, binding the invocation to the validated values without reloading the file; first-run provider/model answers explicitly select a newly created default for that invocation. Profiles cannot persist credentials, approval policy, MCP, execution, commands, workspace, or durable scope. Durable state is scoped by workspace and tenant/user/namespace, uses optimistic revisions and leases, and does not permit incompatible paused approvals to be rebound. Release publication uses an annotated tag, one inspected tarball, checksum verification, protected live certification, npm OIDC, and SLSA provenance.
