...
The coordinated SDK batch pins `@zhivex-ai/agents@1.9.0`, `@zhivex-ai/core@1.23.0`, `@zhivex-ai/meta@0.2.7`, `@zhivex-ai/qwen@0.15.1`, `@zhivex-ai/openai@0.13.5`, and `@zhivex-ai/gemini@0.12.2`. Qwen `0.11.x` normalizes missing, placeholder, and continuation tool-call identifiers inside the adapter; Harness retains its deterministic provider-boundary normalizer as defense in depth until the exact candidate passes the complete release-bound representative matrix. A Core override keeps one runtime contract identity across every adapter, while Core `1.13.0` corrects compaction usage accounting and interaction-group retention. OpenAI `0.11.2` preserves synthetic assistant history in Responses, so the harness no longer requires its compaction transport or accounting workarounds.
