...
The coordinated prerelease SDK batch pins `@zhivex-ai/agents@1.10.0-next.0`, `@zhivex-ai/core@1.24.0-next.0`, `@zhivex-ai/meta@0.2.8-next.0`, `@zhivex-ai/qwen@0.15.3-next.0`, `@zhivex-ai/openai@0.13.6-next.0`, and `@zhivex-ai/gemini@0.12.3-next.0`. These exact `next` versions are prereleases, not Stable promotion evidence. Qwen `0.11.x` normalizes missing, placeholder, and continuation tool-call identifiers inside the adapter; Harness retains its deterministic provider-boundary normalizer as defense in depth until the exact candidate passes the complete release-bound representative matrix. A Core override keeps one runtime contract identity across every adapter, while Core `1.13.0` corrects compaction usage accounting and interaction-group retention. OpenAI `0.11.2` preserves synthetic assistant history in Responses, so the harness no longer requires its compaction transport or accounting workarounds.
