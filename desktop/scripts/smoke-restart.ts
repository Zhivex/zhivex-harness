...
const activeClose=process.argv.includes("--active-close");
...
for(const phase of activeClose?["active-close","cancelled-history"]:["prepare","approve","history"]){
 const child=spawn(executable,[...(packaged?[]:[root]),...(["prepare","active-close"].includes(phase)?["--workspace",workspace]:[]),"--smoke-test","--fixture-restart-phase",phase,"--fixture-close-choices","stay,cancel","--fixture-cli",path.join(root,"../dist/cli.js"),"--report-directory",report],{cwd:output,env:{PATH:process.env.PATH!,HOME:output},stdio:["ignore","pipe","pipe"]});
