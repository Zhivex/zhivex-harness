...
try{
 ...
 const manifest={schemaVersion:1,desktopVersion:meta.version,harnessVersion:runtime.harnessVersion,electronVersion:runtime.electronVersion,platform:"darwin",architecture:"arm64",minimumCompilationTarget:minimum,developerIdSigned:false,notarized:false,published:false,source:{revision,workingTreeModified:dirty},artifact:{file:name,...await digest(image)},contents:{asar:await digest(path.join(app,"Contents/Resources/app.asar")),credentialHelper:await digest(helper)},builtOnMacOS:command("/usr/bin/sw_vers",["-productVersion"])};
