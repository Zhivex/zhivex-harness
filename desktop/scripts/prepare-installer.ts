...
const worker=path.join(app,"Contents/Resources/update-worker-entry.cjs");
if(!(await readFile(worker)).equals(extractFile(path.join(app,"Contents/Resources/app.asar"),"build/update-worker-entry.cjs")))throw new Error("INSTALLER_WORKER_RESOURCE_MISMATCH");
