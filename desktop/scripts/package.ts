...
try {
    ...
    const results = await packager({ dir: stage, out: path.join(root, "out"), name: "Zhivex Harness", appBundleId: "ai.zhivex.harness", appVersion: meta.version.split(/[+-]/)[0], buildVersion: meta.version.split(/[+-]/)[0], extendInfo: {ZhivexDesktopVersion: meta.version}, platform: "darwin", arch: "arm64", electronVersion: meta.devDependencies.electron, overwrite: true, asar: true, extraResource:[path.join(root,"build/credential-store"),path.join(root,"build/update-worker-lock")], prune: false });
