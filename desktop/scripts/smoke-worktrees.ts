...
for(const phase of ["tasks-create","tasks-reopen"]){
 const child=spawn(executable,[...(packaged?[]:[root]),...(phase==="tasks-create"?["--workspace",workspace]:[]),"--smoke-test","--fixture-restart-phase",phase,"--report-directory",report],{cwd:output,env,stdio:["ignore","pipe","pipe"]});let stderr="";child.stderr.on("data",chunk=>stderr+=chunk);child.stdout.resume();const timer=setTimeout(()=>child.kill("SIGKILL"),60000);
...
const evidence={schemaVersion:1,packaged,fixture:true,originalChangesPreserved:true,concurrentTaskIsolation:true,wholeAppRestart:true,reviewedCleanup:true,workersExited:true,phases,evidenceDirectory:report};await writeFile(path.join(report,"report.json"),JSON.stringify(evidence,null,2));console.log(JSON.stringify(evidence));
