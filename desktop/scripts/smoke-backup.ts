...
if (job || handoff) await import("./build-update-worker-lock.js");
...
const child = spawn(path.join(root, "node_modules/electron/dist/Electron.app/Contents/MacOS/Electron"), [path.join(directory, "verify.cjs"), ...(job || handoff ? [path.join(root, "build/update-worker-lock")] : [])], {cwd: directory, env: {PATH: process.env.PATH, HOME: directory, ELECTRON_RUN_AS_NODE: "1"}, stdio: "inherit"});
