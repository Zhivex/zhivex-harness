...
const result = await Bun.build({entrypoints: [path.join(import.meta.dir, handoff ? "update-handoff-runtime.ts" : job ? "update-job-runtime.ts" : process.argv.includes("--recovery") ? "state-transaction-runtime.ts" : "backup-runtime-verification.ts")], outdir: directory, naming: "verify.cjs", format: "cjs", target: "node", plugins: [{name: "native-backup-runtime", setup(builder) {
