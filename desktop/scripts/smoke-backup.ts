...
if (job) await import("./build-update-worker-lock.js");
const metadata = await Bun.file(path.join(root, "../package.json")).json();
const result = await Bun.build({entrypoints: [path.join(import.meta.dir, job ? "update-job-runtime.ts" : process.argv.includes("--recovery") ? "state-transaction-runtime.ts" : "backup-runtime-verification.ts")], outdir: directory, naming: "verify.cjs", format: "cjs", target: "node", plugins: [{name: "native-backup-runtime", setup(builder) {
...
const child = spawn(path.join(root, "node_modules/electron/dist/Electron.app/Contents/MacOS/Electron"), [path.join(directory, "verify.cjs"), ...(job ? [path.join(root, "build/update-worker-lock")] : [])], {cwd: directory, env: {PATH: process.env.PATH, HOME: directory, ELECTRON_RUN_AS_NODE: "1"}, stdio: "inherit"});
