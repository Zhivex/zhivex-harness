...
export class ReviewTickets {
 ...
 issue(sessionId:string,review:ApprovalReview){
  ...
  // OCI approvals stay disabled until their patch/mode previews are implemented.
  return {...review,ticketId,canApprove:review.items.length>0&&review.items.every(item=>item.complete&&["run_check","apply_patch","apply_reviewed_edits","apply_reviewed_replacement"].includes(item.name))};
 }
 consume(ticketId:unknown,approve:unknown,now=Date.now()){
  ...
  if(approve&&review.items.some(item=>!item.complete||!["run_check","apply_patch","apply_reviewed_edits","apply_reviewed_replacement"].includes(item.name)))throw new Error("REVIEW_INCOMPLETE");
