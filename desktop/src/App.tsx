...
export function App() {
    ...
    useEffect(() => { let cancelled=false; void window.harness.providers().then(items=>{if(!cancelled)setProviders(items);}).catch(()=>{if(!cancelled)setError("No se pudo cargar la lista de proveedores. Reabrí la aplicación.");});return()=>{cancelled=true;}; }, []);
    useEffect(() => { void selectProject(() => window.harness.initialProject()); return () => { generation.current++; }; }, []);
    async function credentialsChanged() {
        if (!context) return;
        const key=context.project.key, epoch=generation.current;
        setLoading(true);
        try { const next=await window.harness.openProject(key); if(epoch===generation.current){setContext(next);setDisconnected(false);} }
    ...
    useEffect(() => {
        if (!context || !session || modelEditing) return; const key = context.project.key, id = session.sessionId, epoch = generation.current; let stopped = false, timer: ReturnType<typeof setTimeout>;
        const poll = async () => {
            try {
    ...
    async function start(value: string) {
        if (!context || !session || !value.trim() || context.credentialConfigured === false || modelEditing || busy || loading || reconcileRequired || disconnected) return;
    ...
    return <main data-ready={ready} data-project-key={context?.project.key} data-session-id={session?.sessionId}>
        <Navigation credentialsChanged={credentialsChanged} providers={providers} projects={projects} context={context} sessions={sessions} selectedSession={session?.sessionId} loading={loading} open={() => void selectProject(() => window.harness.chooseProject())} selectProject={key => void selectProject(() => window.harness.openProject(key))} selectSession={id => void selectSession(id)} create={() => void createSession()} />
        <section className="chat-workspace"><header><div><span className="eyebrow">{context?.project.name ?? "TU ESPACIO DE TRABAJO"}</span><h1>{session?.title ?? (session ? "Nueva conversación" : "Tu próximo proyecto empieza acá")}</h1></div>{context?.fixture ? <span className="badge">Modelo offline</span> : null}</header>
            <div className="conversation" aria-busy={loading}>{context ? <GitPanel key={`git:${context.project.key}`} projectKey={context.project.key} disabled={loading} /> : null}{context ? <TaskPanel key={context.project.key} context={context} disabled={loading} open={id => selectProject(() => window.harness.openTask(id))} removed={async task => { if (context.task?.id === task.id) await selectProject(() => window.harness.openProject(task.sourceProjectKey)); }} /> : null}{!activity.order.length ? <div className="welcome"><span className="symbol">◈</span><h2>{loading ? "Recuperando estado…" : !context ? "Abrí un repositorio para empezar." : !session ? "Elegí una conversación o creá una nueva." : "¿Qué querés resolver?"}</h2><p>{context?.project.workspace ?? "Tus conversaciones y el estado de las tareas permanecen separados por proyecto."}</p></div> : null}
            ...
            <footer className="chat-composer"><form onSubmit={event => { event.preventDefault(); void start(prompt); }}><label className="composer-label" htmlFor="prompt">Tu mensaje</label><textarea id="prompt" value={prompt} onChange={event => setPrompt(event.target.value)} disabled={!session || loading} placeholder="Pedile a Harness que investigue, implemente o revise…" maxLength={64000} /><ModelSelector key={`${context?.project.key ?? "empty"}:${context?.modelSelection?.provider ?? "openai"}:${context?.modelSelection?.model ?? providers.find(p=>p.id==="openai")?.defaultModel ?? ""}`} context={context} providers={providers} editing={setModelEditing} disabled={loading||busy||run?.status==="waiting_approval"||reconcileRequired||disconnected} changed={next=>{setContext(next);setDisconnected(false);void refreshProjects();}}/><div className="actions"><button type="submit" data-action="start" disabled={!session || context?.credentialConfigured === false || modelEditing || busy || loading || reconcileRequired || disconnected || !prompt.trim()}>Enviar ↗</button>{context?.fixture ? <button type="button" className="secondary" data-action="wait" disabled={!session || busy || reconcileRequired || disconnected} onClick={() => void start("wait-for-cancel")}>Probar espera</button> : null}<button type="button" className="secondary" data-action="cancel" disabled={(!busy && run?.status !== "waiting_approval") || !activeRun.current} onClick={() => void cancel()}>Cancelar</button><button type="button" className="secondary" disabled={!session || loading} onClick={() => { if (session) void selectSession(session.sessionId); }}>Actualizar conversación</button></div></form></footer>
        </section></main>;
}
