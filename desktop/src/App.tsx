...
export function App(){
 ...
 <Conversation key={session?.sessionId??"empty"} activity={activity}/>{context&&session&&run?.status==="waiting_approval"?<ReviewPanel key={`${context.project.key}:${session.sessionId}:${run.runId}:${run.revision}`} projectKey={context.project.key} sessionId={session.sessionId} runId={run.runId} revision={run.revision}/>:null}{run?<p role="status" className="status">Estado: {run.status} · {run.runId}</p>:null}
