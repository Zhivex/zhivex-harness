...
export function credentialCoordinator(deps:{busy():boolean;hosts():Promise<Host[]>;clear():void;configure(provider:string):Promise<CredentialStatus>;delete(provider:string):Promise<CredentialStatus>}){
 let changing=false;
 return {get changing(){return changing;},async change(action:"configure"|"delete",provider="openai"):Promise<CredentialStatus>{
  ...
  try{
   ...
   const result=await deps[action](provider);
