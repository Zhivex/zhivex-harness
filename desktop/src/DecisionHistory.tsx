...
export function DecisionHistory({projectKey,sessionId,runId}:{projectKey:string;sessionId:string;runId:string}){
 ...
 {row.finalDiff?.status==="unavailable"?<p>Diff final no disponible: no hay contenido completo guardado que coincida con la evidencia.</p>:null}
 {row.finalDiff?.status==="complete"?<details className="final-diff" data-action="final-diff"><summary>Ver cambio aplicado</summary><p>Contenido guardado del cambio aplicado en esta decisión.</p>{row.finalDiff.redacted?<p>Algunos datos están ocultos. Los digests corresponden a los archivos originales.</p>:null}{row.finalDiff.files.map(file=><section className="review-file" key={file.path}><h5>{file.path}</h5>{file.beforeMode!==undefined||file.afterMode!==undefined?<p>Permisos: {file.beforeMode?.toString(8)??"ausente"} → {file.afterMode?.toString(8)??"ausente"}</p>:null}<p>Antes</p><pre className="removed">{file.before??"Archivo ausente"}</pre><p>Después</p><pre className="added">{file.after??"Archivo eliminado"}</pre></section>)}</details>:null}
