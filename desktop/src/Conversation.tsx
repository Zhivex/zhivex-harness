...
export function Conversation({activity,projectKey,sessionId}:{activity:ConversationActivity;projectKey?:string|undefined;sessionId?:string|undefined}){
 ...
 return <div ref={root} className="timeline" aria-label="Actividad de la conversación">
 {activity.recovered?<p className="muted">Actividad recuperada desde el snapshot guardado.</p>:null}
 {activity.order.map(id=>{const run=activity.runs[id]!;return <article key={id} data-run={id}>
