...
export function ReviewPanel({projectKey,sessionId,runId,revision}:{projectKey:string;sessionId:string;runId:string;revision:number}){
 ...
 </article>)}<p>La decisión abarca todas las solicitudes mostradas. Rechazar impide estas operaciones; el agente puede continuar con otra propuesta.</p><button type="button" data-action="approve-review" disabled={loading||!review.canApprove} onClick={()=>void decide(true)}>Aprobar solicitudes revisadas</button><button type="button" className="secondary" data-action="deny-review" disabled={loading||!review.items.length} onClick={()=>void decide(false)}>Rechazar solicitudes</button>{!review.canApprove?<p>La aprobación requiere una revisión completa. Los cambios OCI aún requieren un visor de parche y permisos.</p>:null}</>:null}
 </section>;
}
