...
export function ReviewPanel({projectKey,sessionId,runId,revision}:{projectKey:string;sessionId:string;runId:string;revision:number}){
 ...
 {item.files.map((file,index)=><div key={index} className="review-file"><h4>{file.path}</h4><p className="status">Base: {file.expectedDigest??"archivo nuevo"}</p>{file.operation?<p>{file.operation==="delete"?"Eliminar mediante cuarentena recuperable":file.operation==="create"?"Crear archivo":"Actualizar archivo"}</p>:null}{file.beforeMode!==undefined||file.afterMode!==undefined?<p>Permisos: {file.beforeMode===undefined?"ausente":`0o${file.beforeMode.toString(8)}`} → {file.afterMode===undefined?"ausente":`0o${file.afterMode.toString(8)}`}</p>:null}<p>{file.view==="full-file"?"Diff de archivo completo: base verificada y destino propuesto.":file.view==="literal-replacement"?"Reemplazo literal: fragmentos exactos; no representa el archivo completo.":"Contenido de destino completo; la base está vinculada por su digest."}</p>{file.before!==undefined?<pre className="removed">{file.before}</pre>:null}{file.after!==undefined?<pre className="added">{file.after}</pre>:null}</div>)}
 ...
 </article>)}<p>La decisión abarca todas las solicitudes mostradas. Rechazar impide estas operaciones; el agente puede continuar con otra propuesta.</p><button type="button" data-action="approve-review" disabled={loading||!review.canApprove} onClick={()=>void decide(true)}>Aprobar solicitudes revisadas</button><button type="button" className="secondary" data-action="deny-review" disabled={loading||!review.items.length} onClick={()=>void decide(false)}>Rechazar solicitudes</button>{!review.canApprove?<p>La aprobación requiere una revisión completa. Una base desactualizada o incompleta requiere otra revisión.</p>:null}</>:null}
 </section>;
}
