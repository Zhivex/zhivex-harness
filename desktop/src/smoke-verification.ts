...
export async function verifyDesktopSmoke(window:BrowserWindow,runtimes:Map<string,Promise<ProjectRuntime>>,reportDirectory:string){
 ...
 const isolated=await js('typeof require === "undefined" && typeof process === "undefined" && Object.keys(window.harness).sort().join(",") === "chooseProject,command,createTask,events,gitChanges,gitCommit,gitReconcile,gitReviewCommit,gitStage,initialProject,openProject,openTask,projects,removeTask,resolveReview,review,reviewTaskRemoval,tasks"');assert(isolated);
