...
export async function verifyDesktopSmoke(window: BrowserWindow, runtimes: Map<string, Promise<ProjectRuntime>>, reportDirectory: string) {
    ...
    const isolated = await js('typeof require === "undefined" && typeof process === "undefined" && Object.keys(window.harness).sort().join(",") === "checkUpdates,chooseProject,command,configureCredential,createPr,createTask,credentialStatus,deleteCredential,downloadUpdate,events,gitChanges,gitCommit,gitReconcile,gitReviewCommit,gitStage,initialProject,installUpdate,openPr,openProject,openTask,probeCredential,projects,push,reconcilePr,reconcilePush,remoteTargets,removeTask,resolveReview,review,reviewPr,reviewPush,reviewTaskRemoval,tasks,updateStatus"'); assert(isolated);
