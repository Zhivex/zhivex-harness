...
export async function verifyDesktopSmoke(window:BrowserWindow,runtimes:Map<string,Promise<ProjectRuntime>>,reportDirectory:string){
 ...
 const isolated=await js('typeof require === "undefined" && typeof process === "undefined" && Object.keys(window.harness).sort().join(",") === "chooseProject,command,createPr,createTask,events,gitChanges,gitCommit,gitReconcile,gitReviewCommit,gitStage,initialProject,openPr,openProject,openTask,projects,push,reconcilePr,reconcilePush,remoteTargets,removeTask,resolveReview,review,reviewPr,reviewPush,reviewTaskRemoval,tasks"');assert(isolated);
