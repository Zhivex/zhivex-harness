...
export async function verifyDesktopWorktreesSmoke(window:BrowserWindow,runtimes:Map<string,Promise<ProjectRuntime>>,directory:string,phase:string){
 ...
 if(phase==="tasks-create"){
  ...
  await click('.git-panel summary');await click('[data-action="git-refresh"]');await wait('document.querySelector("[data-git-path]") !== null');await click('[data-git-path="review.txt"]');await wait('document.querySelector("[data-action=git-stage]").disabled === false');await click('[data-action="git-stage"]');await wait('document.body.innerText.includes("Staged: seleccionar para revisar")');await click('[data-git-path="review.txt"]');
  await js(`{const field=document.querySelector('[data-field="commit-message"]');Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype,"value").set.call(field,"Commit revisado desde UI");field.dispatchEvent(new Event("input",{bubbles:true}));}`);
  await wait('document.querySelector("[data-action=git-review]").disabled === false');await click('[data-action="git-review"]');await wait('document.querySelector("[data-action=git-commit]")?.disabled === false');await click('[aria-label="Revisión del commit"] details summary');assert(await js('document.body.innerText.includes("task one only") && document.body.innerText.includes("baseline")'));
 ...
 await writeFile(path.join(directory,`${phase}-report.json`),JSON.stringify({phase,packaged:app.isPackaged,appPid:process.pid,runtimePids:workers.map(worker=>worker.context.runtimePid),tasks:saved.map(item=>item.task.id),concurrentRuns:phase==="tasks-create",restartIsolation:phase==="tasks-reopen",reviewedCleanup:phase==="tasks-reopen",gitCommitUI:phase==="tasks-create",lostCommitResponseReconciledAfterReload:phase==="tasks-create",runtimeGitMutationBlocked:phase==="tasks-create",fixture:true}));window.close();
}
