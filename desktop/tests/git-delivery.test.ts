...
test("reviewed commit binds the whole staged selection and preserves unstaged and foreign files",async()=>{const f=await fixture();try{
...
test("foreign staged files, changed review and index locks never produce a commit",async()=>{const f=await fixture();try{
...
test("secret paths, known values, binary previews and conflicts are rejected before projection",async()=>{const f=await fixture();try{
...
test("repository hooks and diff drivers do not execute during review or commit",async()=>{const f=await fixture();try{
...
test("old review cannot survive restart or authorize a changed branch head",async()=>{const f=await fixture();try{
