...
test("native policy requires pinned Developer ID for app and helper, hardened runtime and notarized origin", () => fixture(async app => {
 ...
 const requirements = calls.filter(c => c.args.includes("-R")); expect(requirements).toHaveLength(2);
...
test("unsigned, wrong publisher or unnotarized app fail before returning an accepted candidate", () => fixture(async app => {
 for (const failure of ["signature", "helper", "runtime", "override", "ambiguous-origin", "architecture", "version"]) {
  let seenSignatures = 0;
  const verify = createMacApplicationVerifier(async (binary, args) => {
   if (args.includes("--verify")) {seenSignatures++; if (failure === "signature" || (failure === "helper" && seenSignatures === 2)) throw new Error("synthetic private diagnostic");}
